...
| 1.3        | 19.12.2025  |        | Anpassung Einleitung                              | gematik
| 1.4        | 15.04.2026  |        | Dateinamen geändert                               | gematik
|===
...
[IMPORTANT]
====
Dieses Dokument ist auch Grundlage für nachfolgende Dokumentationen der TI-Messenger Testsuite.

Diese weiterführenden Versionen der Dokumentation (adoc) bauen auf die in diesem Dokument (adoc) etablierten Informationen und Grundlagen auf.

Nachfolgende Dokumentationsversionen besitzen ebenfalls Gültigkeit und sind an die entsprechenden TI-Messenger Versionen gebunden.
====
...
Der BDD-Parameterizer (auch Cucumber-Test-Kombinations-Plugin genannt) wird dazu genutzt die Testdaten-Kombinationen, in diesem Fall die APIs, zu steuern.
...
Die potenziell möglichen Enum Werte sind in der Testtreiber Schnittstellenbeschreibung zu finden:
...
|12.04| Events innerhalb einer Organisation - Gruppenchat - Test-Client "B" sendet eine Nachricht (HBA-User lädt Organisations-User ein)
...
Diese Auslistung dient der Analyse und zeigt exakt, welche Testtreiberschnittstellen welche Aktionen ausführen inkl. deren Response.
...
Folgende Feature Files bzw.
Testfälle behandelt das Thema IOP in einem größeren Rahmen.
