...
==== Select a different combine_items file

By default, the testsuite uses link:{sourcedir}/test/resources/combine_items.json[combine_items.json] to configure test combinations.
A different file can be selected using either a Maven profile or a system property.

The following Maven profiles are available for the pre-defined CI variants:
...
|`ci`
|`src/test/resources/combine_items_ci.json`

|`ci-V2`
|`src/test/resources/combine_items_ci_V2.json`

|`ci-V3`
|`src/test/resources/combine_items_ci_V3.json`

|`ci-11x`
|`src/test/resources/combine_items_ci_11x.json`
|===
...
Alternatively, any arbitrary file path can be specified directly via the `combine.items.file` system property:

----
mvn verify -Dcombine.items.file=/path/to/custom_combine_items.json
----
