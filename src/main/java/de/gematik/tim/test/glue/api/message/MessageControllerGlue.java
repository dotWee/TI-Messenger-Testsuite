...
import static de.gematik.tim.test.glue.api.message.tasks.DeleteMessageTask.deleteMessageWithId;
import static de.gematik.tim.test.glue.api.message.tasks.DeleteMessageViaReplaceTask.deleteReplaceMessageWithId;
import static de.gematik.tim.test.glue.api.message.tasks.EditMessageTask.editMessage;
import static de.gematik.tim.test.glue.api.message.tasks.SendDirectMessageTask.sendDirectMessageTo;
import static de.gematik.tim.test.glue.api.message.tasks.SendDirectMessageToMxIdTask.sendDirectMessageToMxIdOutOfFederation;
...
public class MessageControllerGlue {
  ...
  private static final String GEO_URI = "geo:52.5229073,13.3875704";
  ...
  @Then(
      "{string} could not see message for attachment {string} from {string} in the chat with {string} [Retry {long} - {long}]")
  @Dann(
  ...
  @Dann(
      "{string} kann die Nachricht für das Attachment {string} von {string} im Chat mit {string} nicht mehr sehen [Retry {long} - {long}]")
  public void canNotSeeChatMessageAnymore(
    ...
    cantFindMessageInRoomAnymore(
        actorName, messageText, messageAuthor, roomName, timeout, pollInterval);
  }

  @Then(
      "{string} could not find message {string} from {string} in room {string} [Retry {long} - {long}]")
  @Then(
      "{string} could not find message of attachment {string} from {string} in room {string} [Retry {long} - {long}]")
  @Dann(
  ...
  @Dann(
      "{string} kann die Nachricht für das Attachment {string} von {string} im Raum {string} nicht mehr sehen [Retry {long} - {long}]")
  ...
  @When("{string} deletes their message {string} in chat with {string} with {string}")
  @Wenn("{string} löscht seine Nachricht {string} im Chat mit {string} mit {string}")
  public void deleteMessageInChatWithType(
      String actorName, String messageText, String userName, String deletionType) {
    deleteMessageInRoomWithType(
        actorName,
        messageText,
  ...
  @When(
      "{string} deletes via {string} the message, which was created for the attachment {string} in the chat with {string}")
  @Wenn(
      "{string} löscht mit {string} die Nachricht, die für das Attachment {string} im Chat mit {string} erzeugt wurde")
  public void deleteAttachmentInChatWithType(
      String actorName, String deletionType, String file, String userName) {
    deleteMessageInRoomWithType(
        actorName, file, DIRECT_CHAT_NAME + theActorCalled(userName).recall(MX_ID), deletionType);
  }

  @When(
      "{string} deletes via {string} the message, which was created for the attachment {string} in the room {string}")
  @Wenn(
      "{string} löscht mit {string} die Nachricht, die für das Attachment {string} im Raum {string} erzeugt wurde")
  public void deleteAttachmentInRoomWithType(
      String actorName, String deletionType, String file, String roomName) {
    deleteMessageInRoomWithType(actorName, file, roomName, deletionType);
  }
  ...
  @When("{string} deletes their message {string} in the room with {string} with {string}")
  @Wenn("{string} löscht seine Nachricht {string} im Raum {string} mit {string}")
  public void deleteMessageInRoomWithType(
      String actorName, String messageName, String roomName, String deletionType) {
    if (deletionType.equals("redact")) {
      deleteMessageInRoom(actorName, messageName, roomName);
  ...
  private void checkRequiredProperties(MessageDTO receivedMessage, String msgType) {
    ...
    if (receivedMessage.getFormat() != null && !receivedMessage.getFormat().isEmpty()) {
      ...
      assertThat(receivedMessage.getGeoUri()).isEqualTo(GEO_URI);
    }
  }
  ...
  public void sendMessageToChatWithMsgType(
    ...
    sender.attemptsTo(sendDirectMessageTo(receiver, message).withMsgType(msgType));
    RoomDTO room = checkRoomMembershipStateInDirectChatOf(actorName);
    checkRoomVersion(room);
  ...
  public void sendMessageToRoomWithMsgType(
    ...
    Actor actor = theActorCalled(actorName);
    actor.abilityTo(UseRoomAbility.class).setActive(roomName);
    actor.attemptsTo(sendMessage(message).withMsgType(msgType));
    checkRoomMembershipState(actor, roomName);
    checksForSendMessageWithMsgType(actorName, message, msgType);
  }
  ...
  public void getMessageFromRoomWithMsgType(
    ...
    Optional<MessageDTO> optionalMessage =
        actor.asksFor(messageFromSenderWithTextInActiveRoom(message, senderMxId));
  ...
  public void sendLocationToChatWithMsgType(
    ...
    Actor receiver = theActorCalled(userName);
    sender.attemptsTo(
        sendDirectMessageTo(receiver, location).withMsgType(msgType).withGeoUri(GEO_URI));
    RoomDTO room = checkRoomMembershipStateInDirectChatOf(actorName);
    checkRoomVersion(room);
    checkResponseCode(actorName, OK.value());
  ...
  public void sendLocationToRoomWithMsgType(
    ...
    Actor actor = theActorCalled(actorName);
    actor.abilityTo(UseRoomAbility.class).setActive(roomName);
    actor.attemptsTo(sendMessage(location).withMsgType(msgType).withGeoUri(GEO_URI));
    checkRoomMembershipState(actor, roomName);
    checkResponseCode(actorName, OK.value());
    MessageDTO receivedMessage = getCreatedMessage(location);
    assertThat(receivedMessage.getBody()).isNotNull().isNotEmpty();
  ...
  @Then("{string} requests the messages for room {string} and gets the response code {int}")
  @Dann("{string} bekommt beim Abruf der Nachrichten für Raum {string} den Responsecode {int}")
  public void cannotRequestMessages(String actorName, String roomName, int responseCode) {
    Actor actor = theActorCalled(actorName);
    RoomDTO publicRoom = getRoomByInternalName(roomName);
    addRoomToActor(publicRoom, actor);
    actor.abilityTo(UseRoomAbility.class).setActive(roomName);
    actor.asksFor(messagesInActiveRoom().messagesCanBeFound(false));
    checkResponseCode(actorName, responseCode);
  }
}
